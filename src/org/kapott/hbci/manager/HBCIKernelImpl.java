...
public final class HBCIKernelImpl implements HBCIKernel {
    ...
    public HBCIKernelImpl(IHandlerData parentHandlerData, String hbciversion) {
        ...
        try {
            AbstractHBCICallback callback = (AbstractHBCICallback) HBCIUtilsInternal.getCallback();
            Document syntax = callback.getHBCIDocument(hbciversion);
            if (syntax != null) {
                gen = new MsgGen(syntax);
                currentMsgName = null;
                return;
            }
        ...
        String filename = xmlpath + "hbci_" + hbciversion + ".xml";
        syntaxStream = cl.getResourceAsStream(filename);
        if (syntaxStream == null) {
            throw new InvalidUserDataException(
                    HBCIUtilsInternal.getLocMsg("EXCMSG_KRNL_CANTLOAD_SYN", filename));
        }
        try {
    ...
    public String getHBCIVersion(int dummy) {
        String ret = "";
        NodeList nodes =
                gen.getSyntax().getElementById("MsgHeadUser").getElementsByTagName("value");
    ...
    public HBCIMsgStatus rawDoIt(boolean signit, boolean cryptit, boolean needSig,
                                 boolean needCrypt) {
    ...
    public HBCIMsgStatus rawDoIt(HBCIPassportList passports, boolean signit, boolean cryptit,
                                 boolean needSig, boolean needCrypt) {
        ...
        try {
            ...
            HBCIUtilsInternal.getCallback()
                    .status(mainPassport, HBCICallback.STATUS_MSG_CREATE, currentMsgName);
            ...
            while (tok.hasMoreTokens()) {
                ...
                if (rewriterName.length() != 0) {
                    Class cl = this.getClass().getClassLoader()
                            .loadClass("org.kapott.hbci.rewrite.R" + rewriterName);
            ...
            if (signit) {
                ...
                HBCIUtilsInternal.getCallback()
                        .status(mainPassport, HBCICallback.STATUS_MSG_SIGN, null);
                // signatur erzeugen und an nachricht anhängen
                Sig sig =
                        SigFactory.getInstance().createSig(getParentHandlerData(), msg, passports);
                try {
                    if (!sig.signIt()) {
                        String errmsg = HBCIUtilsInternal.getLocMsg("EXCMSG_CANTSIGN");
                        if (!HBCIUtilsInternal
                                .ignoreError(null, "client.errors.ignoreSignErrors", errmsg)) {
            ...
            if (maxmsgsize != 0 && (outstring.length() >> 10) > maxmsgsize) {
                String errmsg = HBCIUtilsInternal.getLocMsg("EXCMSG_MSGTOOLARGE",
                        new Object[]{Integer.toString(outstring.length() >> 10),
                                Integer.toString(maxmsgsize)});
                if (!HBCIUtilsInternal
                        .ignoreError(null, "client.errors.ignoreMsgSizeErrors", errmsg)) {
                    throw new HBCI_Exception(errmsg);
                }
            }
            ...
            if (cryptit) {
                ...
                HBCIUtilsInternal.getCallback()
                        .status(mainPassport, HBCICallback.STATUS_MSG_CRYPT, null);
                ...
                if (!msg.getName().equals("Crypted")) {
                    ...
                    if (!HBCIUtilsInternal
                            .ignoreError(null, "client.errors.ignoreCryptErrors", errmsg)) {
                        throw new HBCI_Exception(errmsg);
                    }
                }
                ...
                HBCIUtils.log("encrypted message to be sent: " + msg.toString(0),
            ...
            HBCIUtils.log("communicating dialogid/msgnum " + dialogid + "/" + msgnum,
                    HBCIUtils.LOG_DEBUG);
            MSG old = msg;
            ...
            if (crypted) {
                HBCIUtilsInternal.getCallback()
                        .status(mainPassport, HBCICallback.STATUS_MSG_DECRYPT, null);
                ...
                HBCIUtils.log("decrypted message after rewriting: " + newmsgstring,
                        HBCIUtils.LOG_DEBUG2);
                // nachricht als plaintextnachricht parsen
                try {
                    HBCIUtilsInternal.getCallback()
                            .status(mainPassport, HBCICallback.STATUS_MSG_PARSE,
                                    currentMsgName + "Res");
                    HBCIUtils.log("message to pe parsed: " + msg.toString(0), HBCIUtils.LOG_DEBUG2);
                    MSG oldMsg = msg;
                    msg = MSGFactory.getInstance()
                            .createMSG(currentMsgName + "Res", newmsgstring, newmsgstring.length(),
                                    gen);
                    if (msg != oldMsg) {
                        MSGFactory.getInstance().unuseObject(oldMsg);
                    }
                } catch (Exception ex) {
                    throw new CanNotParseMessageException(
                            HBCIUtilsInternal.getLocMsg("EXCMSG_CANTPARSE"), newmsgstring, ex);
                }
            }
            HBCIUtils.log("received message after decryption: " + msg.toString(0),
                    HBCIUtils.LOG_DEBUG2);
            ...
            try {
                ...
                if (!hbciversion2.equals(hbciversion)) {
                    throw new HBCI_Exception(HBCIUtilsInternal.getLocMsg("EXCMSG_INVVERSION",
                            new Object[]{hbciversion2, hbciversion}));
                }
                String msgnum2 = msg.getValueOfDE(msgPath + ".MsgHead.msgnum");
                if (!msgnum2.equals(msgnum)) {
                    throw new HBCI_Exception(HBCIUtilsInternal
                            .getLocMsg("EXCMSG_INVMSGNUM_HEAD", new Object[]{msgnum2, msgnum}));
                }
                msgnum2 = msg.getValueOfDE(msgPath + ".MsgTail.msgnum");
                if (!msgnum2.equals(msgnum)) {
                    throw new HBCI_Exception(HBCIUtilsInternal
                            .getLocMsg("EXCMSG_INVMSGNUM_TAIL", new Object[]{msgnum2, msgnum}));
                }
                String dialogid2 = msg.getValueOfDE(msgPath + ".MsgHead.dialogid");
                if (!dialogid.equals("0") && !dialogid2.equals(dialogid)) {
                    throw new HBCI_Exception(HBCIUtilsInternal
                            .getLocMsg("EXCMSG_INVDIALOGID", new Object[]{dialogid2, dialogid}));
                }
                if (!dialogid.equals("0") && !msg.getValueOfDE(msgPath + ".MsgHead.MsgRef.dialogid")
                        .equals(dialogid)) {
                ...
                String errmsg = HBCIUtilsInternal.getLocMsg("EXCMSG_MSGCHECK") + ": " + HBCIUtils
                        .exception2String(e);
                if (!HBCIUtilsInternal
                        .ignoreError(null, "client.errors.ignoreMsgCheckErrors", errmsg)) {
                    throw e;
                }
            }
            ...
            HBCIUtilsInternal.getCallback()
                    .status(mainPassport, HBCICallback.STATUS_MSG_VERIFY, null);
            boolean sigOk = false;
            ...
            if (currentMsgName.startsWith("DialogEnd") && HBCIUtils.getParam(paramName, "no")
                    .equals("yes")) {
                HBCIUtils.log(e, HBCIUtils.LOG_WARN);
                HBCIUtils
                        .log("error while receiving DialogEnd response - " + "but ignoring it because of special setting",
                                HBCIUtils.LOG_WARN);
            } else {
                ret.addException(e);
            }
    ...
    public void rawSet(String key, String value) {
        HBCIUtils
                .log("setting raw property " + currentMsgName + "." + key + " to \"" + value + "\"",
                        HBCIUtils.LOG_DEBUG2);
